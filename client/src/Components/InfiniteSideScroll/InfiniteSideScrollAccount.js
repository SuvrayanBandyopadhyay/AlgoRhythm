...
const InfiniteSideScrollAccount = ({user_id}) => {
    ...
    const loadMoreItems = useCallback(async () => {
      if(!hasAll){
        try{
          ...
          const response = await fetch(`http://localhost:5000/accountsongs/${user_id}?i=${items.length}`);
...
export default InfiniteSideScrollAccount